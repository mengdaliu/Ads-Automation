...
public class Main {
    ...
    public static AdAccount _account = new AdAccount("1709175122492173", context);
        ...
        for (String[] message : ((ArrayList<String[]>) inputList.get("Messages"))) {
            ...
            for (Object imagePath : inputList.get("Images")) {
                ...
                for (Object bid : inputList.get("Bids")) {
                    ...
                    String campaignID = campaignCreator.CreateCampaign("Adtiger_" + _campaignName + "_geng_" + d + "_TuringBot_" + i);
                    System.out.println("Created Campaign " + i);
                    ...
                    for (Map.Entry<String, String> entry : images.entrySet()) {
                        ...
                        String adCreative = adCreator.createAdCreative("Creative" + j,
                                title, entry.getValue(), msg, ((String[]) inputList.get("PageID").get(0))[0],  ((String[]) inputList.get("AppID").get(0))[1]);
                        String adSet = adCreator.createAdSet("AdSet " + j, Long.parseLong((String) bid), campaignID, inputList.get("Countries"), ((String[]) inputList.get("AppID").get(0))[0], ((String[]) inputList.get("AppID").get(0))[1],device);
                        String ad = adCreator.createAd("Ad " + j, adSet, adCreative);
                        System.out.println("Just created " + j + "th ad for Campaign " + i + ".");
                        } else {
                            for (Object videoName : inputList.get("Videos")) {
                                String vidID;
                                if (videoIDs.containsKey(videoName)) {
                                    vidID = videoIDs.get(videoName);
                                    System.out.println(videoName + " was previously uploaded. Now got its ID.");
                                } else {
                                    System.out.println("Uploading " + videoName);
                                    vidID = _account.createAdVideo().addUploadFile((String) videoName, new File(_videoHeader + videoName)).execute().getId();
                                ...
                                String adCreative = adCreator.createAdCreativeVideo("Creative" + j, title, entry.getValue(), msg, ((String[]) inputList.get("PageID").get(0))[0],  ((String[]) inputList.get("AppID").get(0))[1], vidID, 0);
                                String adSet = adCreator.createAdSet("AdSet " + j, Long.parseLong((String) bid), campaignID, inputList.get("Countries"), ((String[]) inputList.get("AppID").get(0))[0], ((String[]) inputList.get("AppID").get(0))[1],device);
                                String ad = adCreator.createAd("Ad " + j, adSet, adCreative);
                                System.out.println("Just created " + j + "th ad for Campaign " + i + ".");
                            }
                        }
                        System.out.println("Sleeping for 0 second. ");
